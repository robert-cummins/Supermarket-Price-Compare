...
async function scrapeSites() {
    ...
    for (let i = 1; i <= 20; i++) {
        if (i <= 3) { scrapeNewWorldPakSave("https://www.paknsaveonline.co.nz/category/chilled-frozen-and-desserts/desserts?ps=50&pg=", i, context, page, PakAndSaveProduct, 'PakSave') }
        if (i <= 5) { await scrapeNewWorldPakSave("https://www.ishopnewworld.co.nz/category/chilled-frozen-and-desserts/desserts?ps=50&pg=", i, context, page, NewWorldProduct, 'NewWorld') }
        if (i <= 6) {
        ...
        if (i <= 7) { await scrapeNewWorldPakSave("https://www.ishopnewworld.co.nz/category/baby-toddler-and-kids?ps=50&pg=", i, context, page, NewWorldProduct, 'NewWorld') }
        if (i <= 9) {
